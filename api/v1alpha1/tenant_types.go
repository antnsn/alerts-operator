...
type TenantSpec struct {
	...
	// (compile.BackendNamespace -- Loki's ruler 404s on any namespace containing "/"), and a
	// Kubernetes namespace or object name can never contain "_", so "_" is an unambiguous separator
	// only as long as the prefix has none either. Allowing one would make <prefix>_<ns>_<name>
	// ambiguous and would break the segment-boundary ownership match the prune and the finalizer
	// gate their deletes on -- concretely, a Tenant with prefix "team" would own everything starting
	// "team_", which includes every namespace of a Tenant with prefix "team_a", and would prune them
	// (conflictingTenant compares prefixes for equality, so it would not see the two as colliding).
	// With "_" forbidden here that is unreachable: no legal prefix can start with another legal
	// prefix followed by "_". See compile.OwnsNamespace and TestLokiOwnershipRequiresUnderscoreFreePrefix.
	//
	// This is a tightening of the pattern, not a migration: the field is immutable, and nothing had
	// ever been deployed when it changed (v0.1.0 was not yet tagged and the task-24 e2e run ended
	// with `make purge-dev-crds`), so no stored Tenant can carry an "_" prefix.
