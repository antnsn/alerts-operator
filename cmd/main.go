...
// Package main runs the alerts-operator controller manager.
package main
...
func main() {
	...
	if err := (&controller.ContactPointReconciler{
		Client: mgr.GetClient(),
		Scheme: mgr.GetScheme(),
	}).SetupWithManager(mgr); err != nil {
		setupLog.Error(err, "Failed to create controller", "controller", "contactpoint")
		os.Exit(1)
	}
	...
	if err := (&controller.AlertRuleGroupReconciler{
		Client: mgr.GetClient(),
		Scheme: mgr.GetScheme(),
	}).SetupWithManager(mgr); err != nil {
		setupLog.Error(err, "Failed to create controller", "controller", "alertrulegroup")
		os.Exit(1)
	}
