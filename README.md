...
| children | `Accepted` | References resolve (Tenant, backend, Secrets, ContactPoints) **and the object would pass Alertmanager's own loader on its own**: rules parse (`InvalidRule`), a `ContactPoint`'s receiver validates with its real Secret values (`Invalid`, message redacted), a `NotificationPolicy`'s matchers/durations/inhibit rules load (`Invalid`). An `Accepted=False` child is excluded from the Tenant's compile, so one broken object cannot take the rest of the tenant down; a `NotificationPolicy` that routes to a rejected `ContactPoint` is itself `False/ContactPointNotAccepted` (message names the ContactPoint) rather than compiling with a dangling receiver. Reasons: `TenantNotFound`, `BackendNotConfigured`, `SecretNotFound`, `ContactPointNotFound`, `ContactPointNotAccepted`, `Conflict`, `InvalidRule`, `Invalid`. |
