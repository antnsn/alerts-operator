...
- **One Tenant per (`tenantId`, Mimir address) for Alertmanager.** `GET`/`POST`/`DELETE /api/v1/alerts` is scoped by `X-Scope-OrgID` and the backend URL only — never by `rulesNamespacePrefix` — so two Tenants sharing those two fields address one single document, whatever their prefixes. A `POST` replaces it whole, so writing it under a shared claim would destroy the other Tenant's routing. The operator refuses: both Tenants report `AlertmanagerSynced=False/Conflict` naming each other, emit an `AlertmanagerOwnershipConflict` event, go `Ready=False/Conflict`, and the live document is left frozen at whatever was written last. Rules are unaffected — different prefixes own different rule namespaces, so both keep syncing them. Give one Tenant its own `tenantId` or its own Mimir address to resolve it. (Prefixes exist to let several Tenants share one org's *rules*; they cannot split its Alertmanager config.)
